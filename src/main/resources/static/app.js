...
let currentRoute = 'dashboard'; let records = []; let vendorId = '—'; let currentPage = 1; const pageSize = 10; const selectedOrderLines = new Map();
...
function renderDocument(route) {
  ...
  page.innerHTML = `<div class="page-header"><div><p>LIVE SAP QUERY</p><h1>${resource.title}</h1><span>${resource.description}</span></div><div class="page-actions">${actions}<button class="btn btn-secondary" id="exportBtn">⇩ 导出</button><button class="btn btn-primary" id="pageRefresh">↻ 刷新数据</button></div></div><section class="card"><div class="filter-bar"><label class="search-box">⌕ <input id="keyword" placeholder="输入单据号、物料或关键字查询" /></label><select id="statusFilter"><option value="">全部状态</option><option value="待确认">待确认</option><option value="已确认">已确认</option><option value="待交货">待交货</option><option value="部分收货">部分收货</option><option value="部分完成">部分完成</option><option value="已完成">已完成</option><option value="已取消">已取消</option></select><input id="dateFrom" type="date" title="开始日期" /><input id="dateTo" type="date" title="结束日期" /></div><div id="tableMeta" class="table-meta">正在读取 SAP 实时数据…</div><div class="table-scroll"><table class="data-table"><thead id="tableHead"></thead><tbody id="tableBody"><tr><td class="loading" colspan="8">正在读取 SAP 实时数据…</td></tr></tbody></table></div><footer id="tableFooter" class="table-footer"></footer></section>`;
  $('#pageRefresh').addEventListener('click', () => loadRecords(route)); $('#exportBtn').addEventListener('click', exportCsv); $('#keyword').addEventListener('input', () => { currentPage = 1; renderTable(route); }); $('#statusFilter').addEventListener('change', () => { currentPage = 1; renderTable(route); }); ['dateFrom', 'dateTo'].forEach((id) => $(`#${id}`).addEventListener('change', () => { currentPage = 1; renderTable(route); }));
...
function renderTable(route) {
  ...
  $('#tableBody').innerHTML = shown.length ? pageRows.map((row, index) => `<tr>${route === 'purchaseOrders' ? `<td class="select-col"><input type="checkbox" data-select-line="${index}" ${selectedOrderLines.has(lineKey(row)) ? 'checked' : ''} aria-label="选择订单行" /></td>` : ''}${keys.map((key) => `<td>${cell(key, row[key])}</td>`).join('')}<td><button class="table-link" data-detail="${index}">查看详情</button>${route === 'purchaseOrders' ? ` <button class="table-link" data-create-asn="${index}">创建 ASN</button>` : ''}</td></tr>`).join('') : `<tr><td class="loading" colspan="${keys.length + 1 + (route === 'purchaseOrders' ? 1 : 0)}">暂无符合条件的数据。请核对 SAP 供应商编码与接口实体配置。</td></tr>`;
  $('#tableFooter').innerHTML = `<span>共 ${shown.length} 条记录 · 第 ${currentPage} / ${totalPages} 页</span><div class="pagination"><button class="page-button" data-page="${currentPage - 1}" ${currentPage === 1 ? 'disabled' : ''}>‹ 上一页</button><span>${currentPage}</span><button class="page-button" data-page="${currentPage + 1}" ${currentPage === totalPages ? 'disabled' : ''}>下一页 ›</button></div>`;
  page.querySelectorAll('[data-detail]').forEach((button) => button.addEventListener('click', () => openDrawer(pageRows[Number(button.dataset.detail)], route)));
  page.querySelectorAll('[data-create-asn]').forEach((button) => button.addEventListener('click', () => openModal([pageRows[Number(button.dataset.createAsn)]])));
  page.querySelectorAll('[data-select-line]').forEach((checkbox) => checkbox.addEventListener('change', () => { const row = pageRows[Number(checkbox.dataset.selectLine)]; checkbox.checked ? selectedOrderLines.set(lineKey(row), row) : selectedOrderLines.delete(lineKey(row)); updateBulkAsn(); }));
  page.querySelectorAll('[data-page]').forEach((button) => button.addEventListener('click', () => { currentPage = Number(button.dataset.page); renderTable(route); }));
  const all = $('#selectAll'); if (all) { all.checked = pageRows.length > 0 && pageRows.every((row) => selectedOrderLines.has(lineKey(row))); all.addEventListener('change', () => { pageRows.forEach((row) => all.checked ? selectedOrderLines.set(lineKey(row), row) : selectedOrderLines.delete(lineKey(row))); renderTable(route); }); } updateBulkAsn();
}
